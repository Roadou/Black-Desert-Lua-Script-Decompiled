...
local function Acquire_Animation()
  ...
  Acquire_UI.servantSkillIcon:SetFontAlpha(alphaValue)
  return ret
end
        ...
        do
          ...
          if not isEnable then
            ...
            if Acquire_Enum.LearnSkill == notifyMsg._msgType then
              table.insert(skillInfo._learnSkill, skillSSW:getSkillNo())
              break
            end
            if Acquire_Enum.SkillLearnable == notifyMsg._msgType then
              table.insert(skillInfo._learnableSkill, skillSSW:getSkillNo())
              break
            else
              break
            end
...
local function Acquire_SetData(notifyMsg)
  ...
  elseif Acquire_Enum.ProductLevelUp == arcType then
    ...
    arcObjectMsg = PAGetStringParam1(Defines.StringSheet_GAME, productLevelUpObejctMessage[arcParam], "level", getCraftLevelString(Int64toInt32(notifyMsg._Param2)))
...
function AcquireUpdate(updateTime)
  if nil ~= Panel_Widget_LocalQuestAlert and true == Panel_Widget_LocalQuestAlert:GetShow() then
    if Acquire_ConstValue.showTime > Acquire_Value.elapsedTime then
      Acquire_Value.elapsedTime = Acquire_ConstValue.showTime
    end
...
function getCraftLevelString(lev)
  local strLevel = ""
  if lev >= 1 and lev <= 10 then
    strLevel = "<PAColor0xFFC4C4C4>" .. PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_CRAFTLEVEL_GROUP_1") .. lev .. "<PAOldColor>"
  elseif lev >= 11 and lev <= 20 then
    strLevel = "<PAColor0xFF76B24D>" .. PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_CRAFTLEVEL_GROUP_2") .. lev - 10 .. "<PAOldColor>"
  elseif lev >= 21 and lev <= 30 then
    strLevel = "<PAColor0xFF3B8BBE>" .. PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_CRAFTLEVEL_GROUP_3") .. lev - 20 .. "<PAOldColor>"
  elseif lev >= 31 and lev <= 40 then
    strLevel = "<PAColor0xFFEBC467>" .. PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_CRAFTLEVEL_GROUP_4") .. lev - 30 .. "<PAOldColor>"
  elseif lev >= 41 and lev <= 50 then
    strLevel = "<PAColor0xFFD04D47>" .. PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_CRAFTLEVEL_GROUP_5") .. lev - 40 .. "<PAOldColor>"
  elseif lev >= 51 and lev <= 80 then
    strLevel = "<PAColor0xFFB23BC7>" .. PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_CRAFTLEVEL_GROUP_6") .. lev - 50 .. "<PAOldColor>"
  elseif lev >= 81 and lev <= 100 then
    strLevel = "<PAColor0xFFC78045>" .. PAGetString(Defines.StringSheet_GAME, "LUA_CHARACTERINFO_CRAFTLEVEL_GROUP_7") .. lev - 80 .. "<PAOldColor>"
